package 图论._10最大人工岛._827_最大人工岛_广搜版;
...
class Solution {
    ...
    public int largestIsland(int[][] grid) {
        ...
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                ...
                if (grid[row][col] == 1) {
                    int areaSize = 1 + bfs(grid, row, col, mark);
    ...
    private int bfs(int[][] grid, int row, int col, int mark) {
        int ans = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] current : position) {
                ...
                if (!inArea(grid, nextRow, nextCol)) continue;
